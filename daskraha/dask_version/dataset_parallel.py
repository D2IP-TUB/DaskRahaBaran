...
from daskraha.dataset import Dataset as Data
...
class DatasetParallel(Data):
    ...
    def initialize_dataset(
        ...
        """
        if create_frame:
            self.create_shared_dataframe(
                self.dirty_path, self.dirty_mem_ref, dataset=self
            )
        if create_clean:
        ...
        if create_frame and create_clean and create_diffs:
            differences_dict = self.get_dataframes_difference(
                self.load_shared_dataframe(self.dirty_mem_ref),
    ...
    def create_shared_dataset(dataset):
        ...
        shared_mem_area = sm.SharedMemory(
            name=dataset.own_mem_ref, create=True, size=pickled_dataset_size
        )
    ...
    def create_shared_dataframe(dataframe_filepath, mem_area_name, dataset=None):
        ...
        dataframe = dask.dataframe.read_csv(
            urlpath=dataframe_filepath, blocksize=int(blocksize), **kwargs
        ).applymap(DatasetParallel.value_normalizer)
        ...
        shared_mem_area = sm.SharedMemory(
            name=mem_area_name, create=True, size=pickled_dataframe_size
        )
    ...
    def create_shared_split_dataframe(dataframe_ref):
        ...
        for column in dataframe.columns.tolist():
            pickled_dataframe = pickle.dumps(
                dataframe[column], protocol=pickle.HIGHEST_PROTOCOL
            )
            pickled_dataframe_size = len(pickled_dataframe)
            try:
                shared_mem_area = sm.SharedMemory(
                    name=column, create=True, size=pickled_dataframe_size
                )
                shared_mem_area.buf[:pickled_dataframe_size] = pickled_dataframe
                shared_mem_area.close()
            except FileExistsError:
                shared_mem_area = sm.SharedMemory(
                    name=column, create=False, size=pickled_dataframe_size
                )
                shared_mem_area.buf[:pickled_dataframe_size] = pickled_dataframe
                shared_mem_area.close()
            except Exception as shm_err:
                print(
                    "Failed to create or attach to a split dataframe: {}".format(
                        shm_err
                    )
                )
                raise
    ...
    def write_csv(
        ...
        if dataframe_ref is not None:
            DatasetParallel.load_shared_dataframe(dataframe_ref).to_csv(
                destination_path, sep=",", header=True, index=False, encoding="utf-8"
            )
    ...
    def get_correction_dictionary(self):
        ...
        """
        return self.get_dataframes_difference(
            self.read_csv_dataframe(self.dirty_path),
    ...
    def get_data_quality(self):
        ...
        return 1.0 - float(len(self.get_actual_errors_dictionary())) / (
            self.read_csv_dataframe(self.dirty_path).shape[0]
            * self.read_csv_dataframe(self.dirty_path).shape[1]
        )
