...
import daskraha
import daskraha.constants as constants
import daskraha.dask_version.container as container
import daskraha.dask_version.dataset_parallel as dp
from daskraha.constants import *
from daskraha.detection import Detection
...
class DetectionParallel(Detection):
    def __init__(self):
        ...
        self.ERROR_DETECTION_ALGORITHMS = [
            ...
            KNOWLEDGE_BASE_VIOLATION_DETECTION,
        ]
    ...
    def initialize_dataset(self, dd):
        ...
        differences_dict = dp.DatasetParallel.get_dataframes_difference(
            dirty_dataframe, clean_dataframe
        )
    ...
    def run_outlier_strategy(self, configuration, dataset_ref, strategy_name_hash):
        ...
        daskraha.dask_version.tools.dBoost.dboost.imported_dboost.run(parameters)

        dboost_result_path = dataset_path + "-dboost_output.csv"
        if (
            os.path.exists(dboost_result_path)
            and os.path.getsize(dboost_result_path) > 0
        ):
            dboost_dataframe = pandas.read_csv(
                dboost_result_path,
                sep=",",
                header=None,
                encoding="utf-8",
    ...
    def run_rule_strategy(self, configuration, dataset_ref):
        ...
        left_attribute_j = dp.DatasetParallel.get_column_names(
            dataset.dirty_path
        ).index(left_attribute)
        right_attribute_j = dp.DatasetParallel.get_column_names(
            dataset.dirty_path
        ).index(right_attribute)
    ...
    def run_knowledge_strategy(self, configuration, dataset_ref):
        ...
        """
        dataset = dp.DatasetParallel.load_shared_dataset(dataset_ref)
        outputted_cells_katara = daskraha.dask_version.tools.KATARA.katara.run(
    ...
    def parallel_strat_runner_process(self, args):
        ...
        strategy_name_hashed = str(
            int(hashlib.sha1(strategy_name.encode("utf-8")).hexdigest(), base=16)
        )

        match algorithm:
            case constants.OUTLIER_DETECTION:
                # Run outlier detection strategy
                outputted_cells = self.run_outlier_strategy(
                    configuration, dataset_ref, strategy_name_hashed
                )
            ...
            case constants.KNOWLEDGE_BASE_VIOLATION_DETECTION:
                ...
                outputted_cells = self.run_knowledge_strategy(
        ...
        if self.SAVE_RESULTS:
            ...
            pickle.dump(
                ...
                open(
                    os.path.join(
                        dataset.results_folder,
                        "strategy-profiling",
                        strategy_name_hashed + ".dictionary",
                    ),
                    "wb",
                ),
            )
    ...
    def setup_outlier_metadata(dataset_ref):
        ...
        configurations.extend(
            [dataset_ref, OUTLIER_DETECTION, conf] for conf in cartesian_config
        )
    ...
    def pattern_violation_worker(dataset_ref, column_name):
        ...
        configurations.extend(
            [dataset_ref, PATTERN_VIOLATION_DETECTION, conf]
            for conf in character_dict_list
        )
    ...
    def setup_pattern_violation_metadata(dataset_ref):
        ...
        futures.append(
            client.map(
                DetectionParallel.pattern_violation_worker, arguments1, arguments2
            )
        )
    ...
    def setup_rule_violation_metadata(dataset_ref):
        ...
        column_pairs = [
            [col1, col2]
            for (col1, col2) in itertools.product(column_names, column_names)
            if col1 != col2
        ]

        configurations.extend(
            [
                [dataset_ref, RULE_VIOLATION_DETECTION, column_pair]
    ...
    def setup_knowledge_violation_metadata(dataset_ref):
        ...
        paths = [
            os.path.join(
                os.path.dirname(__file__), "tools", "KATARA", "knowledge-base", path
            )
            for path in os.listdir(
                os.path.join(
                    os.path.dirname(__file__), "tools", "KATARA", "knowledge-base"
                )
            )
        ]
        configurations.extend(
            [[dataset_ref, KNOWLEDGE_BASE_VIOLATION_DETECTION, path] for path in paths]
        )
    ...
    def run_strategies(self, dataset):
        ...
        """
        start_time = time.time()
        strategy_profile_path = os.path.join(
            dataset.results_folder, "strategy-profiling"
        )
        client = get_client()
        futures = []
        ...
        if self.STRATEGY_FILTERING:
            for data_dictionary in self.HISTORICAL_DATASETS + [dataset.dictionary]:
                daskraha.original.utilities.dataset_profiler(data_dictionary)
                daskraha.original.utilities.evaluation_profiler(data_dictionary)
            return (
                daskraha.original.utilities.get_selected_strategies_via_historical_data(
                    dataset.dictionary, self.HISTORICAL_DATASETS
                )
            )

        if os.path.exists(strategy_profile_path) and self.PRELOADING:
            sys.stderr.write(
                "Preloading strategies' results, as they have already been run on the dataset\n"
            )
            strategy_profiles = [
                pickle.load(
                    open(os.path.join(strategy_profile_path, strategy_file), "rb")
                )
        ...
        for algorithm_name in self.ERROR_DETECTION_ALGORITHMS:
            match algorithm_name:
                case constants.OUTLIER_DETECTION:
                    futures.append(
                        client.submit(
                            DetectionParallel.setup_outlier_metadata,
                            dataset.own_mem_ref,
                        )
                    )
                case constants.PATTERN_VIOLATION_DETECTION:
                    futures.append(
                        client.submit(
                            DetectionParallel.setup_pattern_violation_metadata,
                            dataset.own_mem_ref,
                        )
                    )
                case constants.RULE_VIOLATION_DETECTION:
                    futures.append(
                        client.submit(
                            DetectionParallel.setup_rule_violation_metadata,
                            dataset.own_mem_ref,
                        )
                    )
                case constants.KNOWLEDGE_BASE_VIOLATION_DETECTION:
                    futures.append(
                        client.submit(
                            DetectionParallel.setup_knowledge_violation_metadata,
                            dataset.own_mem_ref,
                        )
                    )
                case _:
                    raise ValueError(
                        "Algorithm " + str(algorithm_name) + " is not supported!"
                    )

        # Gather Results of all workers, metadata configuration
        results = list(
            itertools.chain.from_iterable(client.gather(futures=futures, direct=True))
        )
        end_time = time.time()
        self.TIME_TOTAL += end_time - start_time
        if self.VERBOSE:
            print(
                "Raha strategy metadata generation(parallel): "
        ...
        for j in range(dataset.dataframe_num_cols):
            ...
            dp.DatasetParallel.create_shared_object(
                strategy_profiles_col, dataset.dirty_mem_ref + "-s_p-c" + str(j)
            )
        ...
        if self.VERBOSE:
            print(
                "Raha running all strategies total time(parallel): "
                + str(end_time - start_time)
            )
    ...
    def generate_features_one_col(self, dataset_ref, column_index):
        ...
        feature_vectors = numpy.zeros(
            (dataset.dataframe_num_rows, len(strategy_profiles))
        )
        ...
        if self.TFID_ENABLED:
            ...
            column_name = dp.DatasetParallel.get_column_names(dataset.dirty_path)[
                column_index
            ]
            ...
            try:
                ...
                feature_vectors = numpy.column_stack(
                    (feature_vectors, numpy.array(tfid_features.todense()))
                )
            except:
                pass

        promising_strategies = numpy.any(
        ...
        dp.DatasetParallel.create_shared_object(
            feature_vectors, dataset.dirty_mem_ref + "-f-r-c" + str(column_index)
        )
    ...
    def generate_features(self, dataset):
        ...
        futures.append(
            client.map(
                self.generate_features_one_col,
                [dataset.own_mem_ref] * dataset.dataframe_num_cols,
    ...
    def build_clusters_single_column(self, dataset_ref, column_index):
        ...
        try:
            ...
            clustering_model = fastcluster.linkage_vector(
                column_features, method="single", metric="cosine"
            )
        ...
        if self.VERBOSE:
            print(
                "A hierarchical clustering model is built for column {}".format(
                    column_index
                )
            )
        clusters_k_j_c_ce = {
            k: clusters_k_c_ce[k] for k in range(2, self.LABELING_BUDGET + 2)
        }
        cells_clusters_k_j_ce = {
            k: cells_clusters_k_ce[k] for k in range(2, self.LABELING_BUDGET + 2)
        }
    ...
    def build_clusters(self, dataset):
        ...
        futures.append(
            client.map(
                self.build_clusters_single_column,
                [dataset.own_mem_ref] * dataset.dataframe_num_cols,
                numpy.arange(dataset.dataframe_num_cols),
            )
        )
    ...
    def sample_tuple(self, dataset):
        ...
        for j in numpy.arange(dataset.dataframe_num_cols):
            for c in dataset.clustering_results[j][1][k]:
                dataset.labels_per_cluster[(j, c)] = {
                    cell: dataset.labeled_cells[cell][0]
        ...
        dataset.sampled_tuple = numpy.random.choice(
            numpy.arange(dataset.dataframe_num_rows), 1, p=p_tuple_score
        )[0]
        if self.VERBOSE:
    ...
    def propagate_labels(self, dataset):
        ...
        dataset.extended_labeled_cells = {
            cell: dataset.labeled_cells[cell][0] for cell in dataset.labeled_cells
        }
        ...
        for j in numpy.arange(dataset.dataframe_num_cols):
            ...
            if cell in dataset.clustering_results[j][2][k]:
                c = dataset.clustering_results[j][2][k][cell]
                dataset.labels_per_cluster[(j, c)][cell] = dataset.labeled_cells[cell][
                    0
                ]
                    ...
                    if len(dataset.labels_per_cluster[(j, c)]) > 0:
                        match self.LABEL_PROPAGATION_METHOD:
                            case constants.HOMOGENEITY:
                                cluster_label = list(
                                    dataset.labels_per_cluster[(j, c)].values()
                                )[0]
                                if sum(dataset.labels_per_cluster[(j, c)].values()) in [
                                    0,
                                    len(dataset.labels_per_cluster[(j, c)]),
                                ]:
                                    for cell in dataset.clustering_results[j][1][k][c]:
                                        dataset.extended_labeled_cells[cell] = (
                                            cluster_label
                                        )
                            case constants.MAJORITY:
                                cluster_label = round(
                                    sum(dataset.labels_per_cluster[(j, c)].values())
                                    / len(dataset.labels_per_cluster[(j, c)])
                                )
                                for cell in dataset.clustering_results[j][1][k][c]:
                            ...
                            case _:
                                raise ValueError(
                                    "The Label Propagation Method"
                                    + str(self.LABEL_PROPAGATION_METHOD)
                                    + " is not supported! Try homogeneity or majority."
                                )
        if self.VERBOSE:
            print(
                "The number of labeled data cells increased from {} to {}.".format(
                    len(dataset.labeled_cells), len(dataset.extended_labeled_cells)
                )
            )
    ...
    @staticmethod
    def predict_labels_single_col(
        classification_model_name, verbose, dataset_ref, column_index
    ):
        """
        ...
        x_train = [
            feature_vectors[i, :]
            for i in numpy.arange(dataset.dataframe_num_rows)
            if (i, column_index) in extended_labeled_cells
        ]
        y_train = [
            extended_labeled_cells[(i, column_index)]
            for i in numpy.arange(dataset.dataframe_num_rows)
            if (i, column_index) in extended_labeled_cells
        ]
    ...
    def predict_labels(self, dataset):
        ...
        dp.DatasetParallel.create_shared_object(
            [dataset.extended_labeled_cells, dataset.labeled_tuples],
        ...
        futures.append(
            client.map(
                ...
                [self.CLASSIFICATION_MODEL] * column_count,
                [self.VERBOSE] * column_count,
                [dataset.own_mem_ref] * column_count,
                numpy.arange(column_count),
            )
        )
        results = client.gather(futures=futures, direct=True)[0]
    ...
    def run(self, dd):
        ...
        while len(dataset_par.labeled_tuples) < self.LABELING_BUDGET:
            ...
            if dataset_par.has_ground_truth:
                self.label_with_ground_truth(
                    dataset_par, differences_dict, clean_dataframe
                )
            # else
        ...
        if self.VERBOSE:
            print(
                "Sampling tuples and labeling with ground truth(parallel): {}".format(
                    end_time - start_time
                )
            )
        ...
        if self.VERBOSE:
            print(
                "Raha Detection took {:.3f} seconds in total.".format(self.TIME_TOTAL)
            )
