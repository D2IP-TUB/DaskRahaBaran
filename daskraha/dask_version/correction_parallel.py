...
class CorrectionParallel(Correction):
    def __init__(self):
        ...
        self.CLASSIFICATION_MODEL = (
            "ABC"  # ["ABC", "DTC", "GBC", "GNB", "KNC" ,"SGDC", "SVC"]
        )
    ...
    def initialize_workers(
        correct_instance, sampled_tuple=None, dataset_ref=None, step=0
    ):
        ...
        if bool(Worker._instances):
            for worker in Worker._instances:
                ...
                worker.correct_instance.label_with_ground_truth(
                    worker.dataset, worker.dataframe, worker.clean_dataframe
                )
    ...
    def initialize_dataframes(self, dictionary):
        ...
        clean_dataframe = dp.DatasetParallel.read_csv_dataframe(
            dictionary["clean_path"]
        )
    ...
    def initialize_models(self, dataset):
        ...
        if os.path.exists(self.PRETRAINED_VALUE_BASED_MODELS_PATH):
            dataset.value_models = pickle.load(
                bz2.BZ2FILE(self.PRETRAINED_VALUE_BASED_MODELS_PATH, "rb")
            )
            if self.VERBOSE:
        ...
        dataset.vicinity_models = {
            j_1: {j_2: {} for j_2 in range(num_cols)} for j_1 in range(num_cols)
        }
        ...
        for row in dataframe.itertuples():
            ...
            for j, value in enumerate(row):
                if (i, j) not in dataset.detected_cells:
                    ...
                    self._vicinity_based_models_updater(
                        dataset.vicinity_models, update_dictionary
                    )
                    self._domain_based_models_updater(
                        dataset.domain_models, update_dictionary
                    )
        if self.VERBOSE:
    ...
    def _domain_based_models_updater(self, model, update_dictionary):
        ...
        self._to_model_adder(
            model, update_dictionary["column"], update_dictionary["new_value"]
        )
    ...
    def _vicinity_based_models_updater(self, models, update_dictionary):
        ...
        for j_1, cell_value in enumerate(update_dictionary["vicinity"]):
            ...
            if cell_value != self.IGNORE_SIGN:
                ...
                self._to_model_adder(
                    models[j_1][j_2], cell_value, update_dictionary["new_value"]
                )
    ...
    def _value_based_models_updater(self, models, update_dictionary):
        ...
        if self.ONLINE_PHASE or (
            update_dictionary["new_value"]
            and len(update_dictionary["new_value"]) <= self.MAX_VALUE_LENGTH
            and update_dictionary["old_value"]
            and len(update_dictionary["old_value"]) <= self.MAX_VALUE_LENGTH
            and update_dictionary["old_value"] != update_dictionary["new_value"]
            and update_dictionary["old_value"].lower() != "n/a"
            and not update_dictionary["old_value"][0].isdigit()
        ):
            remover_transformation = {}
            adder_transformation = {}
            replacer_transformation = {}
            s = difflib.SequenceMatcher(
                None, update_dictionary["old_value"], update_dictionary["new_value"]
            )
            for tag, i1, i2, j1, j2 in s.get_opcodes():
                ...
                if tag == "insert":
                    adder_transformation[index_range] = update_dictionary["new_value"][
                        j1:j2
                    ]
                if tag == "replace":
                    replacer_transformation[index_range] = update_dictionary[
                        "new_value"
                    ][j1:j2]
            for encoding in self.VALUE_ENCODINGS:
                encoded_old_value = self._value_encoder(
                    update_dictionary["old_value"], encoding
                )
                if remover_transformation:
                    self._to_model_adder(
                        models[0], encoded_old_value, json.dumps(remover_transformation)
                    )
                if adder_transformation:
                    self._to_model_adder(
                        models[1], encoded_old_value, json.dumps(adder_transformation)
                    )
                ...
                self._to_model_adder(
                    models[3], encoded_old_value, update_dictionary["new_value"]
                )
                ...
                if encoded_value_string in model:
                    ...
                    if model_name in ["remover", "adder", "replacer"]:
                        for transformation_string in model[encoded_value_string]:
                            index_character_dictionary = {
                                i: c for i, c in enumerate(ed["old_value"])
                            }
                            ...
                            for change_range_string in transformation:
                                ...
                                if model_name in ["adder", "replacer"]:
                                    ov = (
                                        ""
                                        if change_range[0]
                                        not in index_character_dictionary
                            ...
                            pr = (
                                model[encoded_value_string][transformation_string]
                                / sum_scores
                            )
    ...
    def sample_tuple(self, dataset):
        ...
        for j in column_errors:
            for cell in column_errors[j]:
                if cell not in dataset.corrected_cells:
                    ...
                    self._to_model_adder(
                        remaining_column_erroneous_values, j, dataframe.iloc[cell]
                    )
        ...
        for j in remaining_column_erroneous_cells:
            for cell in remaining_column_erroneous_cells[j]:
                ...
                column_score = math.exp(
                    len(remaining_column_erroneous_cells[j]) / len(column_errors[j])
                )
                cell_score = math.exp(
                    remaining_column_erroneous_values[j][value]
                    / len(remaining_column_erroneous_cells[j])
                )
                tuple_score[cell[0]] *= column_score * cell_score
        dataset.sampled_tuple = numpy.random.choice(
            numpy.argwhere(tuple_score == numpy.amax(tuple_score)).flatten()
        )
    ...
    def update_models(self, dataset):
        ...
        cleaned_sampled_tuple = [
            dataset.labeled_cells[(dataset.sampled_tuple, j)][1]
            for j in numpy.arange(dataset.dataframe_num_cols)
        ]
        ...
        for j in numpy.arange(dataset.dataframe_num_cols):
            ...
            if cell_label == 1:
                ...
                if cell not in detected_cells:
                    ...
                    self._to_model_adder(column_errors, cell[1], cell)
                self._value_based_models_updater(
                    dataset.value_models, update_dictionary
                )
                self._domain_based_models_updater(
                    dataset.domain_models, update_dictionary
                )
                ...
                update_dictionary["vicinity"] = [
                    cv if j != cj else self.IGNORE_SIGN
                ...
                update_dictionary["vicinity"] = [
                    cv
                    if j != cj
                    and dataset.labeled_cells[(dataset.sampled_tuple, cj)][0] == 1
                    else self.IGNORE_SIGN
                    for cj, cv in enumerate(cleaned_sampled_tuple)
                ]
            self._vicinity_based_models_updater(
        ...
        if self.VERBOSE:
            print(
                "The error corrector models are updated with new labeled tuple {}.".format(
                    dataset.sampled_tuple
                )
            )
    ...
    def generate_features(self, dataset, cell):
        ...
        error_dictionary = {
            "column": cell[1],
            "old_value": vicinity[cell[1]],
            "vicinity": vicinity,
        }
        value_corrections = self._value_based_corrector(
            dataset.value_models, error_dictionary
        )
        vicinity_corrections = self._vicinity_based_corrector(
            dataset.vicinity_models, error_dictionary
        )
        domain_corrections = self._domain_based_corrector(
            dataset.domain_models, error_dictionary
        )
        models_corrections = (
            value_corrections + vicinity_corrections + domain_corrections
        )
        ...
        for mi, model in enumerate(models_corrections):
            for correction in model:
                if correction not in corrections_features:
                    corrections_features[correction] = numpy.zeros(
                        len(models_corrections)
                    )
    ...
    def summarize_features_one_col(self, chunk_results):
        ...
        if self.VERBOSE:
            print(
                "{} pairs of (a data error, a potential correction) are featurized.".format(
                    pairs_counter
                )
            )
    ...
    def generate_and_predict(self, column_workers, column_errors, step):
        ...
        for j in column_errors:
            if j not in cols_chunks:
                cols_chunks[j] = []
            for cell in column_errors[j]:
                cols_chunks[j].append(
                    cell
                )  # Input dataframe.iloc[cell[0], :] instead of 1 for !!!!!

        predict_futures = []
        for column_index in column_workers:
            predict_futures.append(
                client.submit(
                    self.predict_corrections_column,
                    cols_chunks[column_index],
                    column_index,
                    step,
                )
            )
            ...
            match mode:
                case "normal":
                    predictions = classification_model.predict(
                        list(pair_features[cell].values())
                    )
                case "ones":
    ...
    def predict_corrections_column(self, cells_list, column_index, step):
        ...
        for cell in cells_list:
            if cell in dataset.detected_cells:
                if (
                    cell in dataset.labeled_cells
                    and dataset.labeled_cells[cell][0] == 1
                ):
        ...
        if x_train:
            ...
            pair_feature_chunk_refs = client.map(
                CorrectionParallel.chunk_prediction_process,
                chunks,
                [mode] * len(chunks),
                [classification_model] * len(chunks),
                [step] * len(chunks),
            )

            secede()
            corrected_cells_chunks = client.gather(
                futures=pair_feature_chunk_refs, direct=True
            )
            rejoin()
    ...
    def collect_corrections(self, column_prediction_futures, corrected_cells):
        ...
        """
        client = get_client()
        corrected_cells_cols = client.gather(
            futures=column_prediction_futures, direct=True
        )
    ...
    def predict_corrections(self, d):
        ...
        client = get_client()
        corrected_cells_cols = client.gather(
            futures=d.column_prediction_futures, direct=True
        )
    ...
    def run(self, d):
        ...
        while len(dataset.labeled_tuples) < self.LABELING_BUDGET:
            ...
            dataset.column_prediction_futures = self.generate_and_predict(
                column_workers, dataset.column_errors, step
            )
